...
import { withThousands, formatUSDCAmount2dp, formatUSDCAmount2dpTruncated } from '@/lib/utils'
...
function tracked<T>(fn: () => Promise<T>): [() => Promise<T>, { ok: boolean }] {
  ...
  return [async () => { const r = await fn(); status.ok = true; return r }, status]
}
...
export function useCreditLine({ pollMs = 15_000 }: Options = {}) {
  ...
  const [scoreDisplay, setScoreDisplay] = React.useState<string>(`—/${MAX_SCORE}`)
  ...
  // -------- Cooldown --------
  const [nextBorrowTimeRaw, setNextBorrowTimeRaw] = React.useState<bigint | null>(
    null,
  )
  ...
  const read = React.useCallback(async () => {
    ...
    try {
      ...
      setError((e as any)?.message ?? t('hooks.useCreditLine.errors.readFailed'))
